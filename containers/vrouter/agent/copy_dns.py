...
def main():
    ...
    resolve_proxy = system_bus.get_object(RESOLVED_BUS_NAME,
                                          RESOLVED_OBJ_PATH)
    resolve_iface = dbus.Interface(object=resolve_proxy,
                                   dbus_interface=RESOLVED_MANAGER_IFACE)

    phys_iface_obj_path = get_iface_obj_path(resolve_iface, phys_iface_index)

    phys_iface_proxy = system_bus.get_object(RESOLVED_BUS_NAME,
                                             phys_iface_obj_path)
    ...
    virt_iface_obj_path = get_iface_obj_path(resolve_iface, virt_iface_index)

    virt_iface_proxy = system_bus.get_object(RESOLVED_BUS_NAME,
                                             virt_iface_obj_path)
